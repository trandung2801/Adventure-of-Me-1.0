#include "monster_obj_1.h"
#include "main_obj.h"
#include "GeneralData.h"

MonsterObj1::MonsterObj1()
{
    width_frame_ = 0;
    height_frame_ = 0;
    x_val_ = 0;
...
bool MonsterObj1::LoadImg(std::string path, SDL_Renderer* screen)
{
...
SDL_Rect MonsterObj1::GetRectFrame()
{
    SDL_Rect rect;
    rect.x = rect_.x ;
    rect.y = rect_.y ;
...
void MonsterObj1::Show(SDL_Renderer* des)
{
    if(come_back_time == 0)
    {
...
void MonsterObj1::DoPlayer(Map& gMap)
{
    if(come_back_time == 0 )
    {
        x_val_ = 0;
...
void MonsterObj1::CheckMap(Map& map_data)
{
    int x1 = 0;
    int x2 = 0;
...
void MonsterObj1::ImpMoveType(SDL_Renderer* screen)
{
    ...
    if(type_move_ == MOVE_IN_MONTER)
    {
        if(on_ground_ == true)
        {
            if(x_pos_ > animation_b_)
            {
                input_type_.left_ = 1;
                input_type_.right_ = 0;
                LoadImg("Data IMG//main_animation//Walk//monster 1 left.png", screen);
            }
            else if (x_pos_ < animation_a_)
            {
                input_type_.left_ = 0;
                input_type_.right_ = 1;
                LoadImg("Data IMG//main_animation//Walk//monster 1 right.png", screen);
            }
        }
        else
        {
    ...
    if(type_move_ == FIGHT_MONTER_2)
    {
        if(on_ground_ == true)
        {
            if(x_pos_ > animation_b_)
            {
                input_type_.left_ = 1;
                input_type_.right_ = 0;
                LoadImg("Data IMG//main_animation//Walk//skill 1 left.png", screen);
            }
            else if (x_pos_ < animation_a_)
            {
                input_type_.left_ = 0;
                input_type_.right_ = 1;
                LoadImg("Data IMG//main_animation//Walk//skill 1 right.png", screen);
            }
        }
    }
    if(type_move_ == FIGHT_MONTER_2)
...
void MonsterObj1::InitAttackEffect(attack_effect* p_attack_effect, SDL_Renderer * screen)
{
...
void MonsterObj1::RemoveAttackEffect(const int& idx)
{
...
void MonsterObj1::MakeAttackEffect(SDL_Renderer* screen, const int& x_limit, const int& y_limit)
{
...
void MonsterObj1::DameSkill_1_2()
{
    blood -=10;
}

void MonsterObj1::DameSkill_3()
{
    blood -=250;
}

void MonsterObj1::DameSkill_4()
{
    blood -=1000;
}
